...
def phase1_assign_particles_to_cells(positions: torch.Tensor, cell_size: float):
    ...
    particle_cell_indices = torch.empty(N, dtype=torch.int32, device=device)
    BLOCK_SIZE = 256
...
def phase3_find_cell_boundaries(particle_cell_indices: torch.Tensor, num_cells: int):
    cell_offsets = torch.zeros(num_cells + 1, dtype=torch.int32, device=device)
...
def build_verlet_list(positions, r_verlet, r_verlet_sq):
    cell_size = r_verlet
    ...
    neighbor_counts_sorted = torch.empty(N, dtype=torch.int32, device=device)
    grid = (num_cells,)
    ...
    verlet_offsets_sorted = torch.zeros(N + 1, dtype=torch.int32, device=device)
    verlet_offsets_sorted[1:] = torch.cumsum(neighbor_counts_sorted, dim=0)
    total_neighbors = verlet_offsets_sorted[-1].item()
    verlet_indices_sorted = torch.empty(total_neighbors, dtype=torch.int32, device=device)
...
def calculate_forces_and_potential_verlet(positions, verlet_indices, verlet_offsets, sorted_indices, r_cutoff_sq, r0, c):
    sorted_positions = positions[sorted_indices]
    forces_sorted = torch.zeros_like(sorted_positions)
    potential_per_i_sorted = torch.zeros(N, dtype=dtype, device=device)
    grid = (N,)
...
def run_verlet_list_simulation_general(
    ...
    **kwargs
    ...
    """
    with torch.no_grad():
        # GEÄNDERT: n_particles ist jetzt eine lokale Variable
        num_save_points, n_particles = t_values.size(0), q0.size(0)
        
        # GEÄNDERT: Globale N-Zuweisungen entfernt
        # global N
        # N = n_particles
